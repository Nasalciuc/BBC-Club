...
export default tseslint.config(
  { ignores: ["**/node_modules/**", "**/dist/**", "**/graphify-out/**", "**/*.generated.*", "packages/ui/src/tokens.ts", "apps/mobile/tailwind.theme.js"] },
  ...
  { rules: {
      ...
      "no-restricted-syntax": ["error",
        { selector: "TSAsExpression > MemberExpression[object.object.name='process'][object.property.name='env']", message: "process.env.X as string is forbidden — use loadEnv()." },
        { selector: "CallExpression[callee.property.name='forEach'] > ArrowFunctionExpression[async=true]", message: "forEach(async …) does not await. Use for…of." },
        { selector: "CallExpression[callee.property.name='forEach'] > FunctionExpression[async=true]", message: "forEach(async …) does not await. Use for…of." },
      ],
  } },

  // ── intra-module layering (eslint-plugin-boundaries) ───────────────────────
  { files: ["packages/modules/**/*.ts"], plugins: { boundaries },
  ...
  { files: ["packages/shared/src/api/**/*.ts"], plugins: { bbc }, rules: { "bbc/no-member-id-in-request-schemas": "error" } },

  // ── mobile: design system discipline + React Compiler ──────────────────────
  { files: ["apps/mobile/**/*.{ts,tsx}", "packages/ui/**/*.{ts,tsx}"], plugins: { bbc },
    rules: {
      ...
      "no-restricted-imports": ["error", { paths: [
        ...
        { name: "react", importNames: ["useMemo", "useCallback", "memo"], message: "React Compiler is ON; manual memoization is forbidden." },
        { name: "@react-native-async-storage/async-storage", message: "Use MMKV (cache) or SecureStore (secrets)." },
