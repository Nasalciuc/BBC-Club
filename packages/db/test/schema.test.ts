...
import { rateLimits } from "../../../modules/platform/src/infrastructure/schema";
...
describe("proposals.offers invariants", () => {
  ...
  it("refuses price <= 0", async () => {
    ...
    ).rejects.toThrow(/offers_price_pos/);
  });
  it("refuses published_price below price", async () => {
    ...
    ).rejects.toThrow(/offers_published_gte_price/);
  });
  it("refuses targeting=user without target_member_id (true XOR)", async () => {
    ...
    ).rejects.toThrow(/offers_targeting_consistent/);
  });
  it("refuses a second offer with the same idempotency key", async () => {
    ...
    await expect(db.insert(offers).values({ ...base, idempotencyKey: base.idempotencyKey + "d" })).rejects.toThrow(
      /offers_idempotency/,
    );
  });
  it("refuses an unknown enum value even via raw SQL", async () => {
    await expect(db.execute(sql`UPDATE proposals.offers SET status = 'activ' WHERE false`)).rejects.toThrow(
