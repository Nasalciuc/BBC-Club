...
export const offers = proposals.table("offers", {
  id: id(),
  ...
  index("offers_expiry").on(t.validUntil).where(sql`${t.status} = 'active'`),  // expire cron
  index("offers_target_member").on(t.targetMemberId).where(sql`${t.targetMemberId} IS NOT NULL`),
  ...
  check("offers_iata", sql`length(${t.routeFrom}) = 3 AND length(${t.routeTo}) = 3 AND ${t.routeFrom} <> ${t.routeTo}`),
...
export const offerTargets = proposals.table("offer_targets", {
  offerId: uuid("offer_id").notNull().references(() => offers.id, { onDelete: "cascade" }),  // same module → FK allowed
  memberId: text("member_id").notNull(),
  createdAt: createdAt(),
}, (t) => [
  primaryKey({ columns: [t.offerId, t.memberId] }),
  index("offer_targets_member").on(t.memberId),
]);
