...
export const offerResponses = engagement.table("offer_responses", {
  ...
  index("responses_unsynced").on(t.createdAt).where(sql`${t.syncedToCrm} = false AND ${t.response} = 'interested'`),
  index("responses_member").on(t.memberId, t.createdAt),
  check("responses_sync_consistent", sql`(${t.syncedToCrm} = false) OR (${t.crmActivityId} IS NOT NULL AND ${t.syncedAt} IS NOT NULL)`),
]);
