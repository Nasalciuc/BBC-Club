...
export type CrmConnector = {
  findByEmail(emailNormalized: string): Promise<{ crmClientId: string; fullName?: string; homeAirport?: string } | null>;
  createActivity(input: { externalId: string; memberId: string; offerId: string; kind: "interested" | "dismissed" }): Promise<{ id: string | null }>;
};

/** Mock until Dan grants access. The http adapter and the mirror sync are stage 5. */
export function mockCrm(known: { email: string; crmClientId: string; fullName?: string; homeAirport?: string }[] = []): CrmConnector {
  return {
    async findByEmail(e) {
    ...
    async createActivity(i) { return { id: `mock_${i.externalId}` }; },
  };
}
