...
describe("requireMember — no dead guards", () => {
  let app: Hono<any>; let cookie: string;
  beforeAll(async () => {
  ...
  it("401 with garbage cookie", async () => expect((await app.request("/v1/me", { headers: { Cookie: "bbc.session_token=nope" } })).status).toBe(401));
  it("200 with a verified session", async () => expect((await app.request("/v1/me", { headers: { Cookie: cookie } })).status).toBe(200));
});
