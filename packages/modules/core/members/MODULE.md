...
**Facade:** `getProfile(actor)`, `updateProfile(actor, patch)`, `setPreferences(actor, prefs)`, `getStatus(memberId)`, `timezoneOf(memberId)`, `activeMemberIds()`, `preferencesOf(memberId)` (for notifications dispatch).
