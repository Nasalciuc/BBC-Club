...
export function authorize(permission: Permission, opts: { module?: string; flags?: Flags; log?: (o: object, m: string) => void } = {}): MiddlewareHandler<PrincipalVars> {
  return async (c, next) => {
    ...
    if (p.kind === "system" && p.source === "internal-secret" && !c.req.path.startsWith("/v1/internal/")) return c.json(err("FORBIDDEN"), 403);
    // killswitch = module-level authorization
    if (opts.module && opts.flags && (await opts.flags.isKilled(opts.module))) return c.json(err("SERVICE_DISABLED"), 503);
    await next();
  };
}
