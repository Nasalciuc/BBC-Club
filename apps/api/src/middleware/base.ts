...
export function installBaseMiddleware(app: Hono<any>, opts: { appOrigin: string; mobileScheme: string; metrics: { inc(n: string, l?: Record<string, string>): void; observe(n: string, v: number, l?: Record<string, string>): void } }) {
  app.use("*", requestId());
  app.use("*", secureHeaders());
  app.use("*", timing());
  app.use("/api/auth/*", cors({ origin: [opts.appOrigin, `${opts.mobileScheme}://`], allowHeaders: ["Content-Type", "Authorization", "Cookie"], allowMethods: ["POST", "GET", "OPTIONS"], credentials: true }));
