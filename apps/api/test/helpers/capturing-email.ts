...
export function capturingEmail(): EmailSender & { lastOtp(to: string): string; sent: { to: string; otp: string; purpose: OtpPurpose }[]; failNext(times?: number): void } {
  const sent: { to: string; otp: string; purpose: OtpPurpose }[] = [];
  let failures = 0;
  return {
    sent,
    async sendOtp(input) {
      if (failures > 0) { failures--; throw new Error("Postmark 503: simulated outage"); }
      sent.push(input);
    },
    lastOtp(to) {
