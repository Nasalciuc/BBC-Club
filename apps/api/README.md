...
`buildApp()` does six things and nothing else: env → db + platform → modules in layer order (ports satisfied from earlier facades) → middleware in fixed order → routes (Better Auth, module routes, jobs) → poller. `shutdown()` stops the poller after the in-flight delivery and closes the pool; SIGTERM calls it.

**Killswitch:** a killed module still runs `init` and registers its **facade** so dependents (e.g. `mobile-bff` needing `engagement`) can boot; its HTTP routes, event consumers, and jobs are **not** registered (those URLs answer 404). Pending deliveries for that module pause via flags. Canonical test: `test/boot.test.ts` (respond → 404, proposals feed still 200). Do not cascade-skip dependents.
