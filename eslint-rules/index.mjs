...
export default {
  rules: {
    "no-member-id-in-request-schemas": {
      meta: {
        type: "problem",
        docs: { description: "Request schemas must not accept memberId — the actor comes from the session." },
        schema: [],
      },
      create(ctx) {
        return {
          Property(node) {
            ...
            if (["memberId", "member_id", "userId", "user_id", "ownerId"].includes(key)) {
              ctx.report({
                node,
                message: `"${key}" in a request schema is an IDOR vector. The actor is c.get("principal"), never client input.`,
              });
            }
          },
        };
    ...
    "no-inline-color": {
      meta: {
        type: "problem",
        docs: { description: "Colors come from @bbc/ui tokens, never inline hex/rgba." },
        schema: [],
      },
      create(ctx) {
        ...
        return {
          Literal(node) {
            if (typeof node.value === "string" && re.test(node.value))
              ctx.report({ node, message: "Inline color. Use tokens.colors.*" });
          },
          TemplateElement(node) {
            if (re.test(node.value.raw)) ctx.report({ node, message: "Inline color in template. Use tokens.colors.*" });
          },
        };
      },
    },
