...
describe("authorization gates", () => {
  ...
  it("inventory: every /v1 route is registered with a permission or marked public", () => {
    ...
    expect([...routeRegistry.entries()].filter(([, p]) => p === "public").map(([k]) => k)).toEqual(["GET /v1/app-config"]);
  });
  ...
  it("IDOR: member A cannot see or act on member B's targeted offer (404, body identical to a missing id)", async () => {
    ...
    const missing = await t.app.request(`/v1/proposals/00000000-0000-4000-8000-000000000000`, { headers: { Cookie: t.memberA.cookie } });
    expect(asA.status).toBe(404); expect(await asA.text()).toBe(await missing.text());
    const respondAsA = await t.app.request(`/v1/proposals/${offerForB}/respond`, { method: "POST", headers: { Cookie: t.memberA.cookie, "Content-Type": "application/json" }, body: JSON.stringify({ response: "interested" }) });
  ...
  it("memberId in the body is ignored: A responding 'as B' records A", async () => {
    ...
    await t.app.request(`/v1/proposals/${broadcast}/respond`, { method: "POST", headers: { Cookie: t.memberA.cookie, "Content-Type": "application/json" }, body: JSON.stringify({ response: "interested", memberId: t.memberB.id }) });
  ...
  it("mark-read on someone else's notification → 404 and no change", async () => {
    ...
    expect(r.status).toBe(404); expect(await t.isRead(nB)).toBe(false);
  });

  it("privilege escalation: member cookie on internal route → 403; operator JWT on internal route → 403; wrong secret → 401", async () => {
    expect((await t.app.request("/v1/internal/offers", { method: "POST", headers: { Cookie: t.memberA.cookie } })).status).toBe(403);
    expect((await t.app.request("/v1/internal/offers", { method: "POST", headers: { Authorization: `Bearer ${t.operatorJwt}` } })).status).toBe(403);
    expect((await t.app.request("/v1/internal/offers", { method: "POST", headers: { "X-Internal-Secret": "nope" } })).status).toBe(401);
  });

  it("internal secret cannot be used outside /v1/internal", async () => {
    expect((await t.app.request("/v1/proposals", { headers: { "X-Internal-Secret": t.internalSecret } })).status).toBe(403);
  });

  it("killswitch: engagement disabled → 503 SERVICE_DISABLED, feed still 200", async () => {
    await t.flags.kill("engagement");
    const broadcast = await t.seedBroadcastOffer();
    expect((await t.app.request(`/v1/proposals/${broadcast}/respond`, { method: "POST", headers: { Cookie: t.memberA.cookie, "Content-Type": "application/json" }, body: JSON.stringify({ response: "interested" }) })).status).toBe(503);
  ...
  it("handler acting for member A cannot mark B's response as synced", async () => {
    ...
    await expect(t.runHandler("crm.onOfferResponded", { type: "offer.responded", version: 1, offerId: broadcast, memberId: t.memberA.id, response: "interested", at: new Date().toISOString() }))
