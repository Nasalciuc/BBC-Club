...
export const notificationsRepo = {
  ...
  async unreadCount(exec: Executor, actorMemberId: string) {
    const [{ count }] = await exec.select({ count: sql<number>`count(*)::int` }).from(n).where(and(eq(n.memberId, actorMemberId), isNull(n.readAt)));
    return count;
  },
  ...
  async markRead(exec: Executor, actorMemberId: string, id: string): Promise<number> {
    const rows = await exec.update(n).set({ readAt: sql`now()` })
