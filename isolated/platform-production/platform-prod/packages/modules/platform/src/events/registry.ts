...
export type HandlerContext = {
  ...
  tx: any;
  event: { id: string; type: string; version: number; aggregateType: string; aggregateId: string; memberId: string | null; occurredAt: Date };
...
export class EventRegistry {
  ...
  definition(type: string) { return this.defs.get(type); }
  consumersOf(type: string): string[] { return [...(this.consumers.get(type)?.keys() ?? [])]; }
  handlerFor(type: string, consumer: string) { return this.consumers.get(type)?.get(consumer); }
  allTypes() { return [...this.defs.keys()]; }
  allConsumers() { return [...this.consumers.values()].flatMap((m) => [...m.keys()]); }
