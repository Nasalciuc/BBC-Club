import { pgSchema, text, jsonb, integer, bigserial, timestamp, index, uniqueIndex, check, primaryKey } from "drizzle-orm/pg-core";
...
export const eventDeliveries = platform.table("event_deliveries", {
  ...
  index("deliveries_ready").on(t.runAfter, t.id).where(sql`${t.status} = 'pending'`),
  index("deliveries_consumer_status").on(t.consumer, t.status),
  index("deliveries_cleanup").on(t.processedAt).where(sql`${t.status} = 'done'`),
...
export const externalInbox = platform.table("external_inbox", {
...
export const jobRuns = platform.table("job_runs", {
  id: bigserial("id", { mode: "bigint" }).primaryKey(),
