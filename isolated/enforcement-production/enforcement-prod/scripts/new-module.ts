...
if (!name || flag !== "--layer" || !LAYERS.includes(layer)) { console.error("usage: bun run new-module <name> --layer core|domain|intelligence|integration"); process.exit(1); }
if (!/^[a-z][a-z-]+$/.test(name)) { console.error("module name must be kebab-case"); process.exit(1); }
...
const files: Record<string, string> = {
  "package.json": JSON.stringify({ name: `@bbc/${name}`, version: "0.1.0", private: true, type: "module",
    exports: { ".": "./src/api/index.ts", "./module": "./src/module.ts", "./schema": "./src/infrastructure/schema.ts" },
    scripts: { "test:unit": "bun test tests/unit", "test:db": "bun test tests/contract tests/handlers", typecheck: "tsc --noEmit" },
    dependencies: { "drizzle-orm": "^1.0.0", zod: "^3.24.0", "@bbc/shared": "workspace:*", "@bbc/db": "workspace:*", "@bbc/platform": "workspace:*" } }, null, 2) + "\n",
...
`,
  "src/application/.gitkeep": "", "src/domain/.gitkeep": "", "src/handlers/.gitkeep": "", "src/ports/.gitkeep": "",
...
`,
  "tests/unit/.gitkeep": "", "tests/handlers/.gitkeep": "",
};
