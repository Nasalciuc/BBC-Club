...
for (const layer of readdirSync(base)) {
  ...
  if (existsSync(join(lp, "package.json"))) modules.push(lp);                       // platform lives at layer root
...
for (const m of modules) {
  ...
  else { const md = readFileSync(join(m, "MODULE.md"), "utf8"); for (const s of REQUIRED_SECTIONS) if (!md.includes(s)) problems.push(`${m}: MODULE.md lacks ${s}`); }
  if (!existsSync(join(m, "src/api/index.ts"))) problems.push(`${m}: src/api/index.ts missing`);
  const tests = join(m, "tests");
  if (existsSync(tests)) for (const f of walk(tests)) if (readFileSync(f, "utf8").includes("it.todo(")) problems.push(`${f}: unfinished it.todo`);
}
if (problems.length) { console.error("module:check FAILED\n  - " + problems.join("\n  - ")); process.exit(1); }
console.log(`module:check OK (${modules.length} modules)`);

function walk(dir: string): string[] { return readdirSync(dir).flatMap((f) => { const p = join(dir, f); return statSync(p).isDirectory() ? walk(p) : p.endsWith(".ts") ? [p] : []; }); }
