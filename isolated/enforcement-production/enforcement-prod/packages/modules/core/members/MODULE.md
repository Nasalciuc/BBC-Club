...
**Consumes:** `member.registered` → create profile + match crm mirror by normalized email; `member.deleted` → hard-delete profile + preferences; `crm.mirror.synced` → link waitlist members whose email appeared.
