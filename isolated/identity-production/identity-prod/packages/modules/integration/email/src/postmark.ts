  ...
  return {
    async sendOtp({ to, otp, purpose }) {
      const res = await f("https://api.postmarkapp.com/email", {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json", "X-Postmark-Server-Token": opts.token },
        body: JSON.stringify({
          From: opts.from,
...
export function consoleSender(log: (m: string) => void = console.log): EmailSender {
  return { async sendOtp({ to, otp, purpose }) { log(`[dev-email] ${purpose} → ${to}: ${otp}`); } };
}
