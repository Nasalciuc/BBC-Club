...
describe("account deletion — zero rows everywhere", () => {
  it("removes auth rows, cascades our schemas via member.deleted, tombstones the journal", async () => {
    ...
    for (const [schema, table] of [["auth","user"],["auth","session"],["members","profile"],["notifications","device_tokens"],["notifications","notifications"],["engagement","offer_responses"]]) {
      const n = (await ctx.db.execute(`SELECT count(*)::int AS n FROM ${schema}."${table}" WHERE ${table === "user" ? "id" : "member_id"} = $1`, [memberId]))[0].n;
