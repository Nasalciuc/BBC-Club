...
if (!/test/i.test(new URL(url).pathname)) { console.error("refusing to reset a non-test database"); process.exit(1); }
...
const tables = (await db.execute(sql`
  ...
  WHERE table_type='BASE TABLE' AND table_schema IN ('platform','auth','members','notifications','proposals','engagement','crm','personalization')
    AND table_name NOT LIKE 'domain_events_%'`) as any[]).map((r) => `"${r.t.split(".")[0]}"."${r.t.split(".")[1]}"`);
